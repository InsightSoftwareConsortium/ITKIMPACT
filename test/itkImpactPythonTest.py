...
# The mask above keeps both layers of the model, and each compared layer carries its own
# loss, weight and channel subset. PCA is a property of the extraction, so it stays per model.
metric.SetDistance(["NCC", "NCC"])
metric.SetLayersWeight([1.0, 1.0])
metric.SetSubsetFeatures([4, 4])
metric.SetPCA([0])
